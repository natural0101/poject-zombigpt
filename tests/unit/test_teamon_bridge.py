...
def test_a_goal_can_only_be_one_of_the_four_tokens() -> None:
    lines = {
        goal: encode(goal_message(request_id="teamon-1", goal=goal)) for goal in VoiceGoal
    }
...
def test_a_transcript_is_clamped_and_bounded_rather_than_trusted() -> None:
    message = decode(
        b'{"v":"1.0","type":"transcript","text":"'
        + b"a" * 900
        + b'","at_ms":-5,"confidence":1.7}'
    )
...
def test_exactly_one_carriage_return_is_stripped_so_a_planted_one_still_fails() -> None:
    framer = LineFramer()
    lines = framer.feed(b'{"v":"1.0","type":"ready"}\r\r\n')
    assert lines == [b'{"v":"1.0","type":"ready"}\r']
...
def test_a_windows_launch_attempted_off_windows_fails_as_a_launch_and_leaks_nothing(
    tmp_path: object,
) -> None:
    ...
    recorder = Recorder()
    bridge = JsonlBridge(
        a_config(command=("/bin/true",)), listener=recorder, system="win32"
    )
...
def test_a_bridge_program_that_exists_as_a_file_is_found_even_off_path(
    tmp_path: object,
) -> None:
...
def test_a_listener_that_raises_does_not_take_the_reader_thread_with_it() -> None:
    ...
    bridge._report(
        BridgeReport(state=BridgeState.RUNNING, reason=BridgeReason.DROPPED, detail="x")
    )
