        ...
        if process.poll() is None:
            ...
            try:
                ...
                with suppress(subprocess.TimeoutExpired):
                    process.wait(timeout=self._config.stop_timeout)
        # Closed after the child is gone, so no thread is parked on them. The
        # ValueError is the one raised by closing a file object another thread
        # is mid-read on; there is nothing left to do about it here.
    ...
    def _read_loop(self, process: subprocess.Popen[bytes], generation: int) -> None:
        ...
        while True:
            try:
                ...
                # The pipe was closed under us by the stop path. Falling out is
                # right; the generation check below suppresses the report.
                break
            if not chunk:
                break
