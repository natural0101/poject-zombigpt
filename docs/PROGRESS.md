...
> | `P5-crafting-building` | **crafting wave done here, awaits live** — `crafting.inspect` (read-only, published) and `crafting.craft` (behind the EXPERIMENTAL `crafting` capability, so withheld until a live run promotes it), one command crafts one item once, success only on the product observed in the inventory, typed refusals `RECIPE_UNKNOWN`/`RECIPE_MATERIALS_MISSING`; 15th kind `craft_item` on a bounded deterministic mission that does not go looting when materials are short; P3 spending what is carried, escalating to P4 whenever the recipe may need a surface (which is every craft on a build whose readout is silent); `learn_recipe` repaired — the mod never published the `unread_recipes` the policy reads, so the goal could not succeed on any machine. Open: **building — no structure is placed by anything in this build**; a live crafting scenario in the playbook; live promotion of the capability |
