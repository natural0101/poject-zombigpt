...
> | `P2-continuous-goal-controller` — long-lived goals, needs arbitration | **wave 1 done here, awaits live** — goal persistence across restart (honest SESSION_TERMINATED for the interrupted ACTIVE), `return_home` (speakable «домой») + `explore_area` missions, goal-status phase/paused/report surface; wave 2 open: needs/reflex arbitration, treat_wounds/rest/sleep kinds, event-driven replan |
