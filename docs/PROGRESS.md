...
> | `P2-continuous-goal-controller` — long-lived goals, needs arbitration | **waves 1–2 done here, awaits live** — goal persistence across restart, `return_home`/`explore_area`, phase/paused/report surface; the mandatory hunger chain deterministic (`satisfy_hunger`/`thirst` rerouted off the LLM), `treat_wounds` (speakable)/`rest_until`/`sleep_until_rested` — 12 kinds, 9 deterministic; wave 3 open: needs-preemption arbitration (interrupt + resume), threat-avoidance routing |
