...
| Field | Value |
| --- | --- |
| archive | `pz-agent-windows-rc` (artifact 9228072302), `pz-agent-windows-v1.0.0-rc1.zip`, 77 entries |
| archive sha256 | `7cf75b74ee2be12fd40a6698af60646775da804455a1926b70a6d793df81b915` |
| source commit | `121cd1ad535a1ca28c3d424681b9baf8713b7484` |
| workflow run | https://github.com/natural0101/poject-zombigpt/actions/runs/31822660801 |
| certified by | `check_release.py --rc` printing `CERTIFIED v1.0.0-rc1: 8 check(s) passed` — archive complete, all 11 wrappers at the root, **both executables in `bin/`**, 76 file digests matching, 8540 of 8603 tests passed with no failures, 31 MCP end-to-end testcases green, and the archive claiming no live-test evidence. The packaged pair also completed an MCP `initialize` over the RPC link with `PATH` reduced to the system directories |
| what this RC does *not* certify | skips moved 49 → 63 here, and the fourteen are the protocol-table agreement added at `1a5feb4`: they run the mod's `Protocol.lua` through a Lua interpreter, and the release runner has none. That is the same condition three other seam checks meet — `test_adapter_args_agreement`, `test_capability_declaration_agreement` and the observation round trip — so **no seam check has ever been part of RC certification**, and the skip count moves whenever a seam gains coverage. All four run on Linux CI and locally; each verifies a contract between the mod and the sidecar that has nothing to do with the host OS, and putting Lua on the release runner would add a dependency to the release path for coverage already taken elsewhere. Stated here so the growing skip count is read as what it is |
